// At the top of menu-fix-direct.js
(function($) {
    // Check if emergency navigation was already added
    $(document).ready(function() {
        // Function to prevent duplication during nuclear rebuild
        function preventMenuDuplication() {
            // Ensure only one HOME link exists
            if ($('li#emergency-home-link').length > 1) {
                $('li#emergency-home-link:gt(0)').remove();
                console.log("Prevented HOME link duplication during menu fixes");
            }
        }
        
        // Run immediately and also after any menu manipulations
        preventMenuDuplication();
        
        // Hook into window events that might trigger menu rebuilds
        $(window).on('load resize', preventMenuDuplication);
        
        // Set interval to periodically check and fix
        setInterval(preventMenuDuplication, 1000);
    });
})(jQuery);
(function($) {
    'use strict';
